...
class PhaseOrderState:
    ...
    @staticmethod
    def _compose_order(upstream_order: List[str], terminal_order: List[str]) -> List[str]:
        labels = ('A', 'B', 'C')
        return [upstream_order[labels.index(label)] for label in terminal_order]

    def apply_g2_blackbox_to_pt3(self) -> None:
        """Derive PT3 from G2 terminal blackbox order."""
        self._overwrite_phase("PT3", self.g2_blackbox_order)

    def apply_pt1_blackbox_to_pt_phases(self, pt1_net_order: List[str]) -> None:
        """Derive PT2 from G1 and PT2 secondary order, and PT1 from PT1 net order."""
        self._overwrite_phase("PT2", self._compose_order(
            self.g1_blackbox_order, self.pt2_sec_blackbox_order))
        self._overwrite_phase("PT1", pt1_net_order)

    def apply_pt2_blackbox_to_pt2(self) -> None:
        """Derive PT2 from G1 order and PT2 secondary blackbox order."""
        self._overwrite_phase("PT2", self._compose_order(
            self.g1_blackbox_order, self.pt2_sec_blackbox_order))
